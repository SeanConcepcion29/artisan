...
class PCConsole {
  ...
  String _ping(String targetIP) {
    ...
    const int initialTTL = 5; // max hops
...
PingResult _traverseNetwork(EthernetPort start, String targetIP, Set<EthernetPort> visited, int ttl) {
  ...
  if (start.connectedPC != null) {
    if (start.connectedPC!.ipAddress == targetIP &&
        _sameSubnet(start.ipAddress!, targetIP, start.subnetMask!)) {
      return PingResult.reachable;
    }

    final res = _traverseNetwork(start.connectedPC!.port, targetIP, visited, ttl - 1);
  ...
  if (start.connectedRouter != null) {
    ...
    for (final p in router.ports) {
      if (p.isUp && p.ipAddress != null && p.subnetMask != null) {
        ...
        if (p.connectedPC != null && p.connectedPC!.ipAddress == targetIP && _sameSubnet(p.ipAddress!, targetIP, p.subnetMask!)) {
          if (kDebugMode) { print("${p.ipAddress} ${targetIP} ${p.subnetMask}"); }
    ...
    for (final p in router.ports) {
      if (p.isUp && p.ipAddress != null && p.subnetMask != null) {
        if (_sameSubnet(p.ipAddress!, targetIP, p.subnetMask!)) {
          final res = _traverseNetwork(p, targetIP, visited, ttl - 1);
          if (res != PingResult.unreachable) return res;
        }
      }
    }
    ...
    for (final route in router.routingTable) {
      if (_sameSubnet(targetIP, route.destination, route.netmask)) {
        ...
        if (gwPort != null) {
          final res = _traverseNetwork(gwPort, targetIP, visited, ttl - 1);
          if (res != PingResult.unreachable) return res;
        }
      }
    }
  ...
  // Connected Switch (forward only if connected devices share subnet with start)
  if (start.connectedSwitch != null) {
    for (final p in start.connectedSwitch!.ports) {
      if (p.isUp && p != start) {
        ...
        // Get IP of device connected to this port
        if (p.connectedPC != null) {
          otherIp = p.connectedPC!.ipAddress;
          otherMask = p.connectedPC!.port.subnetMask;
        }
          ...
          for (final rPort in router.ports) {
            ...
            if (rPort.connectedSwitch == start.connectedSwitch) {
              ...
              if (kDebugMode) {
                print("Router interface on same link: $otherIp / $otherMask");
              }
              break;
            }
          }
        ...
        // Only forward if start’s IP is in same subnet as the connected device’s IP
